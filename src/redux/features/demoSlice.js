...
export const demoAction = createAsyncThunkWithTokenRefresh('demo/demoAction', async (token, currentUser, payload) => {
  const headers = {} // Adjust the value as needed

  return axios.get(`/user_service/api/version`, createAxiosConfig(token, currentUser, headers))
})
...
export const demoPayloadAction = createAsyncThunkWithTokenRefresh(
  ...
  async (token, currentUser, payload) => {
    ...
    return axios.post(`/vendor/uploadInvoice/customExceptin`, payload, createAxiosConfig(token, currentUser, headers))
  }
)
...
export const {resetDemoPayloadAction} = demoSlice.actions
