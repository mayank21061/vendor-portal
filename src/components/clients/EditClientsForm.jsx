...
const EditClientsForm = ({ open, setOpen, rowData }) => {
  console.log(rowData)
  ...
  const validationSchema = yup.object({
    ...
    regAddress: yup.string().required('Warehouse Address is required'),
    regState: yup.string().required('Warehouse State is required'),
    regCity: yup.string().required('Warehouse City is required'),
    regPincode: yup
      .string()
      .matches(/^[0-9]+$/, 'Warehouse Pincode must be digits')
      .length(6, 'Warehouse Pincode must be exactly 6 digits')
              ...
              <Grid item xs={12}>
                <FormGroup>
                  <FormControlLabel
                    ...
                    label='Head Office Address is same as Warehouse Address'
                  />
                </FormGroup>
              </Grid>
              {!checked && (
...
export default memo(EditClientsForm)
